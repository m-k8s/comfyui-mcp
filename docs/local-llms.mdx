...
Your OWN skills reach these lanes the same way: set `COMFYUI_MCP_SKILLS_DIRS` to
one or more directories (path-delimited, like `PATH`) whose sub-directories each
hold a `SKILL.md` with `name` and `description` frontmatter, and they are listed
by `skill_list` after the bundled ones, with their origin, and readable by
`skill_read`. A name clash is settled in favour of the bundled skill.
